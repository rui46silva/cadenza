...
export async function ensureDemoSeed(prisma: PrismaClient) {
  ...
  const [admin, professor, aluno, mariana, tiago, sofia, demo, tags] = await Promise.all([
    prisma.user.upsert({
    ...
    prisma.user.upsert({
      ...
      update: { isAmbassador: true, gender: "FEMININO" },
      create: {
  ...
  const [, [post1, post2, post3, post4, , post6, post7]] = await Promise.all([
  ...
  await Promise.all([
    prisma.comment.upsert({
      ...
      create: {
        ...
        authorId: mariana.id,
        content:
          "Ótima dica da Maria! Eu também recomendo gravares-te a tocar para ouvires onde as notas ainda colam.",
        createdAt: daysAgo(5),
      },
    }),
    prisma.comment.upsert({
  ...
  await Promise.all([
    ...
    prisma.waitlistSignup.createMany({
      data: [
